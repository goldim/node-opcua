...
export class Closed extends State implements IState
{
    ...
    public listen(cb: (err?: Error) => void)
    {
        ...
        this._server._impl.on("error", (err: Error) => {
            debugLog(chalk.red.bold(" error") + " port = " 
                + this._server.port, err);
            this._server.changeState("closed");
            this._end_listen(err);
        });
        this._server._impl.on("listening", () => {
            debugLog("server is listening");
        });
        this._server._impl.listen(this._server.port, /*"::",*/ (err?: Error) => { // 'listening' listener
            debugLog(chalk.green.bold("LISTENING TO PORT "), this._server.port, "err  ", err);
